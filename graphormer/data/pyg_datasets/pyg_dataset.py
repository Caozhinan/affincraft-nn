...
class GraphormerPYGDataset(Dataset):
    def __init__(
        ...
        self.seed = seed
        if train_idx is None and train_set is None and valid_set is None and test_set is None:
        ...
        elif train_set is not None or valid_set is not None or test_set is not None:
            self.num_data = 0
            ...
            if valid_set is not None:
                self.num_data += len(valid_set)
                self.valid_data = self.create_subset(valid_set)
            else:
                self.valid_data = None
            if test_set is not None:
                self.num_data += len(test_set)

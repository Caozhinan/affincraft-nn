...
class AffinCraftNodeFeature(nn.Module):    
    ...
    def hierarchical_pool_masif(self, masif_feat, masif_mask=None):  
        """  
        分层池化 masif 特征  
        """  
        ...
        for i in range(0, n_patches, self.local_pool_stride):  
            end_idx = min(i + self.local_pool_size, n_patches)  
            local_patch = masif_feat[:, i:end_idx, :]  # [n_graph, local_size, 80]  

            if masif_mask is not None:  
                local_mask = masif_mask[:, i:end_idx].to(weight_dtype)  # 确保掩码类型匹配  
                # 掩码感知的局部池化  
                masked_patch = local_patch * local_mask.unsqueeze(-1)  
                valid_counts = local_mask.sum(dim=1, keepdim=True)  
                valid_counts = torch.clamp(valid_counts, min=1.0)  
                local_pooled = masked_patch.sum(dim=1) / valid_counts  # [n_graph, 80]  
            else:  
                local_pooled = local_patch.mean(dim=1)  # [n_graph, 80]  

            # 通过局部编码器 - 数据类型已经匹配  
            local_encoded = self.local_masif_encoder(local_pooled)  # [n_graph, hidden_dim//2]  
            local_features.append(local_encoded)  

        # 第二层：全局池化  
        if local_features:  
            stacked_local = torch.stack(local_features, dim=1)  # [n_graph, n_local_regions, hidden_dim//2]  
            global_pooled = self.attention_based_global_pool(stacked_local)  # [n_graph, hidden_dim//2]  
            final_encoded = self.global_masif_encoder(global_pooled)  # [n_graph, hidden_dim]  
        else:  
            final_encoded = torch.zeros(n_graph, self.hidden_dim, device=masif_feat.device, dtype=weight_dtype)  
    ...
    def forward(self, batched_data):    
        ...
        if self.use_masif and "masif_desc_straight" in batched_data:    
            ...
            # 使用分层池化  
            masif_emb = self.hierarchical_pool_masif(masif_feat, masif_mask)  
...
class AffinCraftAttnBias(nn.Module):    
    ...
    def __init__(self, num_heads=32, hidden_dim=768, n_layers=12):    
        ...
        # 6. 新增：angle和distance特征编码器  
        self.angle_encoder = nn.Sequential(  
            nn.Linear(28, num_heads),  # 28是max_angle数量  
            nn.ReLU(),  
            nn.Linear(num_heads, num_heads)  
        )  
          
        self.multi_dist_encoder = nn.Sequential(  
            nn.Linear(28, num_heads),  # 对应angle的距离特征  
            nn.ReLU(),   
            nn.Linear(num_heads, num_heads)  
        )  
    ...
    def forward(self, batched_data):    
        ...
        # 获取angle和distance特征  
        angle = batched_data.get("angle")  # [n_graph, n_node, n_node, n_angle]  
        dists = batched_data.get("dists")  # [n_graph, n_node, n_node, n_angle]  
        ...
        # 添加angle和distance特征到attention bias  
        if angle is not None and dists is not None:  
            # 对angle和distance特征进行编码  
            weight_dtype = self.angle_encoder[0].weight.dtype  
            angle_emb = self.angle_encoder(angle.to(weight_dtype))  # Remove .float()  
            dist_emb = self.multi_dist_encoder(dists.to(weight_dtype))  # Remove .float() 
              
            # 将angle和distance特征添加到attention bias（跳过图token位置）  
    ...
    def _classify_and_embed_edges(self, edge_index, edge_types, distances, num_ligand_atoms, edge_mask=None):    
        ...
        for batch_idx in range(n_graph):    
            ...
            for edge_idx in range(max_edge_num):    
                ...
                # 距离编码    
                weight_dtype = self.distance_encoder[0].weight.dtype  
              
            # 距离编码 - use dynamic type matching instead of .float()  
                distance_emb = self.distance_encoder(distance.to(weight_dtype)) 
